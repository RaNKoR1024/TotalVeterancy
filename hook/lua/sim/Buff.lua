...
BuffEffects = {
    ...
    ShieldMaxHealth = function(buffDefinition, buffValues, unit, buffName)
        ...
        local val = BuffCalculate(unit, buffName, 'ShieldMaxHealth', shield.spec.ShieldMaxHealth)
        val = math.ceil(val)
    ...
    ShieldRegen = function(buffDefinition, buffValues, unit, buffName)
        local shield = unit:GetShield()
        if not shield then return end
        local valregen = shield.spec.ShieldRegenRate
        valregen = BuffCalculate(unit, buffName, "ShieldRegen", valregen)
    ...
    Damage = function(buffDefinition, buffValues, unit, buffName)
        for i = 1, unit:GetWeaponCount() do
            ...
            if wep.Label ~= 'DeathWeapon' 
            and wep.Label ~= 'DeathImpact'
            and not (wepbp.TargetType == 'RULEWTT_Projectile' and wepbp.WeaponCategory == 'Defense')
            then
    ...
    RateOfFire = function(buffDefinition, buffValues, unit, buffName)
        ...
        for i = 1, unit:GetWeaponCount() do
            ...
            if wep.BeamType.rateOfFireDamageMult then
                wep.BeamType.rateOfFireDamageMult = val
            else 
