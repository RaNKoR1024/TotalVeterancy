...
--             unit.Sync.RegenRate = val
        ...
        elseif atype == 'Damage' then
            for i = 1, unit:GetWeaponCount() do
                ...
                if wep.Label != 'DeathWeapon' and wep.Label != 'DeathImpact' then
					if not (wepbp.TargetType == 'RULEWTT_Projectile' and wepbp.WeaponCategory == 'Defense') then --We dont want buffs on tactical missile defense. It breaks them	
						...
						local wepbp = wep:GetBlueprint()
							...
							local muzval = BuffCalculate(unit, buffName, atype, wepbp.MuzzleVelocity)
        ...
        elseif atype == 'DamageRadius' then
            for i = 1, unit:GetWeaponCount() do
                ...
                local wepbp = wep:GetBlueprint()
                if not (wepbp.WeaponCategory == 'Death' or vals.ByName and not vals.ByName[wepbp.Label] ) then
					if not (wepbp.TargetType == 'RULEWTT_Projectile' and wepbp.WeaponCategory == 'Defense') then --We dont want buffs on tactical missile defence. It breaks them
        ...
        elseif atype == 'MaxRadius' then
            for i = 1, unit:GetWeaponCount() do
                ...
                if not (wepbp.WeaponCategory == 'Death' or vals.ByName and not vals.ByName[wepbp.Label] ) then
                    ...
                    wep.rangeMod = val / wepbp.MaxRadius --Put this back to val/wepbp.maxradius. Gonna fix this at projectile creation						
					end
        ...
        elseif atype == 'RateOfFireBuf' then
            for i = 1, unit:GetWeaponCount() do
                ...
                if not (wepbp.WeaponCategory == 'Death' or vals.ByName and not vals.ByName[wepbp.Label] ) then
                    ...
                    wep.bufRoF = val
					if (wepbp.TargetType == 'RULEWTT_Projectile' and wepbp.WeaponCategory == 'Defense') then
					wep.bufRoF = wep.bufRoF * 1.3 --Lets make tactical missile defence scale faster
					end					
