...
--            self.Txp = 0
            ...
            self.Sync.LevelProgress = self.LevelProgress
            self.Sync.RegenRate = bp.Defense.RegenRate
		if self.XPnextLevel == nil then self.XPnextLevel = 100 end
    end,
...
if bpA.Categories and not (table.find(bpA.Categories,'STRUCTURE')
and table.find(bpA.Categories,'DEFENSE')
and table.find(bpA.Categories,'ANTIMISSILE')
and table.find(bpA.Categories,'ARTILLERY'))
then
...
if bpA.Categories 
then for k,bType in buffTypesNonCombat do --Moved health (and ROF) buffs to here only, all units get this...sorry the naming is back to front with bufftypes
			Buff.ApplyBuff( self, 'Veterancy' .. bType)
	end
end
...
--        LOG('___' .. amount .. repr(self:GetBlueprint().Description))
    ...
    DoTakeDamage = function(self, instigator, amount, vector, damageType)
        if instigator and IsUnit(instigator) and not instigator:IsDead() then
			...
			if bp.Categories 
				and (table.find(bp.Categories,'TECH1')
					or table.find(bp.Categories,'TECH2')
					or table.find(bp.Categories,'TECH3'))
				and not table.find(bp.Categories,'STRUCTURE') 
				and not table.find(bp.Categories,'LAND') 
				and not table.find(bp.Categories,'NAVAL') 
				and not table.find(bp.Categories,'ENGINEER')then
				bp.FlyerMulti = 1
				else
