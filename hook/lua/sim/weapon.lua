        ...
        if proj and not proj:BeenDestroyed() then
            local bp = self:GetBlueprint()
				--Mega IF statement for the BlackOps Aeon Tactical Missile - which has no damage or radius
					if bp.Categories 
								and table.find(bp.Categories,'AEON')
								and table.find(bp.Categories,'PROJECTILE')
								and table.find(bp.Categories,'INDIRECTFIRE')
								and table.find(bp.Categories,'MISSILE') 
								and table.find(bp.Categories,'NOSPLASHDAMAGE')
								and bp.Physics.InitialSpeed == 40
								and bp.Physics.MaxSpeed == 40 
								then
            ...
            if Lifetime == 0 then Lifetime = proj:GetBlueprint().Physics.Lifetime * (self.rangeMod * 2) end --Lets add the *2 to the lifetime, see if that makes tactical missiles fire properly
